...
use ains_axum::{Body, BodyExt, Request, Response, Router, StatusCode};
use ains_runtime::{HttpError, Response as UnifiedResponse};
use tower::ServiceExt;

use ains_axum::middleware::rate_limit_middleware;
...
async fn test_handler(_req: ains_axum::UnifiedRequest) -> Result<UnifiedResponse, HttpError> {
...
async fn request_through_middleware(
    ...
    let app = Router::new()
        .route("/", ains_axum::post(test_handler))
        .layer(ains_axum::from_fn_with_state(guard, rate_limit_middleware));
