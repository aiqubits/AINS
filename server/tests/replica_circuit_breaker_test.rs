...
//!
//! Tests require only `AINS_DATABASE_READ_URLS` to be set (comma-separated
...
//! `AINS_DATABASE_READ_URLS` is unset (local development).
//!
...
fn has_read_replicas() -> bool {
    std::env::var("AINS_DATABASE_READ_URLS")
        .ok()
        .map(|v| !v.is_empty())
        .unwrap_or(false)
}
...
fn load_app_config() -> ains_server::utils::config::AppConfig {
    ains_server::utils::load_config("config.toml", "development").expect("Failed to load config")
}
