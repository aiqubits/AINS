...
async fn create_wechat_state_with_len(
    ...
    let wechat = ains_server::services::wechat::init_wechat_components(
        &app_config.wechat,
        &cache,
        db.clone(),
    );
...
async fn seed_captcha(state: &ains_server::AppState, account_id: &str, openid: &str, code: &str) {
...
async fn test_wx_login_success() {
    ...
    let user = ains_server::services::UserService::new(state.db.clone(), state.cache.clone())
        .create_user(
            ains_server::repositories::user::CreateUserInput {
                email: email.clone(),
...
async fn test_login_with_captcha_mismatched_email() {
    ...
    let user_a = ains_server::services::UserService::new(state.db.clone(), state.cache.clone())
        .create_user(
            ains_server::repositories::user::CreateUserInput {
                email: email_a.clone(),
                password: "Password123!".to_string(),
    ...
    let _user_b = ains_server::services::UserService::new(state.db.clone(), state.cache.clone())
        .create_user(
            ains_server::repositories::user::CreateUserInput {
                email: email_b.clone(),
