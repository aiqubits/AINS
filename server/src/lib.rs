...
// salvo 目录已删除 — 所有 salvo 特定代码已下沉到 ains-salvo crate
pub mod bootstrap;
pub mod migrations;
...
pub type ServerRequest = ains_axum::UnifiedRequest;
#[cfg(feature = "ains-salvo")]
pub type ServerRequest = ains_salvo::UnifiedRequest;

/// Ensure at least one framework feature is enabled.
#[cfg(not(any(feature = "ains-axum", feature = "ains-salvo")))]
compile_error!(
    "At least one framework feature must be enabled: 'ains-axum' (default) or 'ains-salvo'"
);
...
#[cfg(not(feature = "ains-salvo"))]
pub use ains_axum::{UnifiedError, UnifiedResponse, delete, get, post, put};
...
#[cfg(not(feature = "ains-salvo"))]
pub use ains_axum::{AxumRuntime, response_to_axum};

// ── Salvo mode: export SalvoRuntime, routing functions, etc. ────
#[cfg(feature = "ains-salvo")]
pub use ains_salvo::{SalvoRouter, SalvoRuntime, delete, get, post, put};
