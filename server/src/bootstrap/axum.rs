        ...
        if env != "development" {
            tracing::warn!(
                "CORS: no allowed_origins configured in {} environment. \
                 ...
                 Otherwise, set server.allowed_origins in config.toml or via AINS_SERVER__ALLOWED_ORIGINS.",
                env
            );
            ...
            Err(e) => {
                tracing::error!(
                    "CORS: failed to parse allowed_origin '{}': {}. \
                     This origin will be ignored — check your config.toml or AINS_SERVER__ALLOWED_ORIGINS.",
                    origin,
                    e
                );
            }
