...
pub fn setup_panic_handler() {
    ...
    let mode = if cfg!(feature = "ains-salvo") {
        "salvo"
    } else {
        "axum"
    };
...
pub async fn bootstrap(cli_args: CliArgs) -> Result<BootstrapResult> {
    tracing::info!("Starting ains in {} mode", cli_args.env);
    ...
    if cli_args.env != "development" {
        ...
        if is_default {
            anyhow::bail!(
                "JWT secret must be changed from the default value in {} environment! \
                 Set AINS_JWT_SECRET environment variable or update config.toml.",
                cli_args.env
            );
        }
        ...
        if app_config.system_admin_email.to_lowercase() == "admin@ains.local"
    ...
    if email.is_empty() || !validator::ValidateEmail::validate_email(&email) {
        anyhow::bail!(
            "System admin email '{}' is not a valid email address. \
             Set AINS_SYSTEM_ADMIN_EMAIL to a valid email address (e.g. 'admin@example.com').",
    ...
    if let Some(user) = existing {
        ...
        anyhow::bail!(
            "A non-system user already exists with the configured system admin email '{}'. \
             ...
             Please change AINS_SYSTEM_ADMIN_EMAIL or remove the conflicting user.",
            email
        );
    }
