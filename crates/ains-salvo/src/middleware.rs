//! Salvo 生态中间件构建器。
//!
//! 提供与 `ains-axum`（tower-http）能力等价的中间件工厂函数。
//! 提供对称的认证、鉴权、限流中间件。
//! 服务端通过 `ains_salvo::middleware::*` 使用，不直接依赖 `salvo` crate。
//!
//! # CORS 注意事项
//!
...
use ains_runtime::auth::{AuthUser, validate_jwt};
use ains_runtime::middleware::MiddlewareState;
...
/// 获取 jwt_secret 和 token_version 校验，业务逻辑委托给 `ains_runtime`。
///
...
mod tests {
    ...
    fn extract_jwt_cookie_success() {
        ...
        req.headers_mut().insert(
            "cookie",
            "ains_jwt=my-jwt-token; other=cookie".parse().unwrap(),
        );
    ...
    fn extract_jwt_cookie_multiple_cookies() {
        ...
        req.headers_mut().insert(
            "cookie",
            "session=abc; ains_jwt=found-token; lang=en"
                .parse()
                .unwrap(),
        );
