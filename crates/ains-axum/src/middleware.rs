//! Adapter-level middleware for ains-axum.
//!
...
//! delegated to `ains_runtime::middleware` and `ains_runtime::auth`.
...
use ains_runtime::{AuthUser, MiddlewareState, RateLimitGuard, validate_jwt};

/// Authentication middleware — validates JWT from `Authorization` header or `ains_jwt` cookie.
...
pub async fn auth_middleware<S: MiddlewareState + 'static>(
    ...
    // Extract token from Authorization header or ains_jwt cookie
...
mod tests {
    ...
    fn extract_jwt_cookie_success() {
        let req = Request::builder()
            .header("cookie", "ains_jwt=my-jwt-token; other=cookie")
            .body(Body::empty())
            .unwrap();
    ...
    fn extract_jwt_cookie_multiple_cookies() {
        let req = Request::builder()
            .header("cookie", "session=abc; ains_jwt=found-token; lang=en")
            .body(Body::empty())
            .unwrap();
