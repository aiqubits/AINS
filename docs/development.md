...
AINS 通过 `ains-runtime` crate 抽象运行时，业务代码（routes / handlers / services）完全不感知底层框架。

### 原理

```
Runtime trait (crates/ains-runtime/src/runtime.rs)
  ├── AxumRuntime<S> (crates/ains-axum/src/lib.rs)  — feature=ains-axum (default)
  └── SalvoRuntime<S> (crates/ains-salvo/src/lib.rs) — feature=ains-salvo
```
...
```bash
# 默认 Axum
cargo run --package ains-server

# 切换 Salvo
cargo run --package ains-server \
  --no-default-features --features ains-salvo
```
...
```
config.toml 路径           → 环境变量
database_url               → AINS_DATABASE_URL
database_read_urls         → AINS_DATABASE_READ_URLS
server.host                → AINS_SERVER__HOST
server.port                → AINS_SERVER__PORT
server.allowed_origins     → AINS_SERVER__ALLOWED_ORIGINS
database.max_connections   → AINS_DATABASE__MAX_CONNECTIONS
```
...
# 仅服务端
cargo test --package ains-server
