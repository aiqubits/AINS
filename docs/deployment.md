...
本文档介绍 AINS 的三级部署方案：本地开发、Docker Compose 中小型部署、Kubernetes 高可用部署（含 CloudNativePG 读写分离集群）。
...
# config.toml already has CHANGE_ME_POSTGRES_PASSWORD and CHANGE_ME_REDIS_PASSWORD as defaults
# Set custom passwords here if needed, or override via AINS_DATABASE_URL / AINS_REDIS_URL

# 3. 启动（默认 Axum 引擎）
cargo run --package ains-server -- --env development --log-level debug

# 4. 切换 Salvo 引擎（可选）
cargo run --package ains-server --no-default-features --features ains-salvo \
...
| 服务 | 容器 | 端口 | 依赖 |
|------|------|------|------|
| ains-server | 后端 (Axum) | 3000 | postgres, redis |
| ains-web | Nginx + WASM | 80 | ains-server |
...
cat > .env << EOF
AINS_ENV=production
AINS_JWT_SECRET=${JWT_SECRET}
AINS_POSTGRES_PASSWORD=${DB_PASS}
AINS_REDIS_PASSWORD=${REDIS_PASS}
EOF
...
```
用户 → nginx:80 (ains-web)
         ├── ains-server:3000 → postgres:5432
         └──                      → redis:6379
```

所有容器在 `ains-net` 网络内通过服务名互访。

### 数据持久化

```bash
# 备份 PG
docker compose exec -T postgres pg_dump -U postgres -d ains > backup.sql
# 恢复
cat backup.sql | docker compose exec -T postgres psql -U postgres -d ains
...
# 单实例 PG 的 URL
DATABASE_URL="postgres://postgres:${POSTGRES_PASS}@postgres-service.ains.svc.cluster.local:5432/ains"
# CNP 集群的 URL（需要同时配置读库地址）
DATABASE_RO_URL="postgres://postgres:${POSTGRES_PASS}@postgres-cluster-ro.ains.svc.cluster.local:5432/ains"

REDIS_URL="redis://:${REDIS_PASS}@redis-service.ains.svc.cluster.local:6379"
...
docker build -t ains-server:latest -f Dockerfile.server .
docker build -t ains-web:latest -f Dockerfile.web .
minikube image load ains-server:latest ains-web:latest
```
...
kubectl wait --for=condition=ready pod -l app=redis -n ains --timeout=120s
```
...
kubectl wait --for=condition=ready pod -l app=postgres -n ains --timeout=300s
```
...
kubectl wait --for=condition=ready cluster -n ains postgres-cluster --timeout=300s
```
...
```bash
DATABASE_URL="postgres://postgres:${POSTGRES_PASS}@postgres-cluster-rw.ains.svc.cluster.local:5432/ains"
# 注意：database_read_urls 在 ConfigMap 中配置，非 Secret
kubectl create secret generic ains-secrets --dry-run=client -o yaml \
  --from-literal=jwt_secret="$JWT_SECRET" \
  --from-literal=postgres_password="$POSTGRES_PASS" \
  --from-literal=redis_password="$REDIS_PASS" \
  --from-literal=database_url="$DATABASE_URL" \
  --from-literal=redis_url="$REDIS_URL" \
  -n ains | kubectl apply -f -
```
...
```bash
# 3 副本后端应用
kubectl apply -f k8s/ains.yml
kubectl rollout status deployment/ains -n ains --timeout=300s

# 前端 Nginx（可选）
kubectl apply -f k8s/ains-web.yml

# Ingress
kubectl apply -f k8s/ingress.yml
```

**关键配置**（[k8s/ains.yml](../k8s/ains.yml)）：
...
```
Ingress → ains-web:80 (2 replicas)
           → ains-server:3000 (3 replicas)
...
```bash
# 手动扩展
kubectl scale deployment ains --replicas=5 -n ains

# HPA 自动伸缩
kubectl autoscale deployment ains --min=3 --max=10 --cpu-percent=70 -n ains

# 滚动更新
kubectl set image deployment/ains ains=your-registry/ains-server:v2 -n ains
kubectl rollout status deployment/ains -n ains

# 金丝雀发布（需要服务网格）
kubectl create deployment ains-canary --image=your-registry/ains-server:v2 -n ains
```
...
database_url = "postgres://user:pass@primary:5432/ains"

# 多 URL → 启用 AutoRouter
database_url = "postgres://user:pass@primary:5432/ains"
database_read_urls = [
  "postgres://user:pass@replica1:5432/ains",
  "postgres://user:pass@replica2:5432/ains",
]
...
```bash
# ConfigMap 中配置读库端点
kubectl patch configmap ains-config -n ains -p \
  '{"data":{"AINS_DATABASE_READ_URLS":"postgres://postgres:${POSTGRES_PASS}@postgres-cluster-ro.ains.svc.cluster.local:5432/ains"}}'
kubectl rollout restart deployment/ains -n ains
```
...
```bash
# 数据库
AINS_DATABASE_URL=postgres://user:pass@host:port/dbname
AINS_DATABASE_READ_URLS=postgres://user:pass@replica1:5432/dbname  # 逗号分隔或数组 JSON

# 数据库路由（JSON 编码）
AINS_DATABASE_ROUTING={"strategy":"round_robin","retry_attempts":2,"circuit_break_ms":30000}

# Redis
AINS_REDIS_URL=redis://:password@host:6379

# JWT
AINS_JWT_SECRET=your-secret-key   # >= 32 chars
AINS_JWT_EXPIRY_SECONDS=3600

# 服务器
AINS_SERVER__HOST=0.0.0.0
AINS_SERVER__PORT=3000
AINS_SERVER__ALLOWED_ORIGINS=https://domain1.com,https://domain2.com

# 日志
RUST_LOG=info|debug|trace

# 环境
AINS_ENV=development|production
```
...
- **最小 3 副本**：`k8s/ains.yml` 默认 `replicas: 3`，容忍单节点故障
- **Pod 反亲和**：避免同一节点多个副本
...
```bash
kubectl exec -it pod/ains-xxx -n ains -- \
  psql -h postgres-service -U postgres -d ains
```
...
```bash
# 检查集群状态
kubectl cnpg status postgres-cluster -n ains

# 检查 Service 端点
kubectl get endpoints -n ains | grep postgres-cluster

# 获取主库 Pod 名称
kubectl get pod -n ains -l postgresql=postgres-cluster \
  -o jsonpath='{.items[?(@.metadata.labels.role=="primary")].metadata.name}'

# 手动测试连接
kubectl exec -it pod/ains-xxx -n ains -- \
  sh -c 'apt-get update && apt-get install -y postgresql-client && \
  psql "$AINS_DATABASE_URL" -c "SELECT 1"'

kubectl exec -it pod/ains-xxx -n ains -- \
  sh -c 'psql -h postgres-cluster-ro -U postgres -d ains -c "SELECT 1"'
```

### Redis 连接失败

```bash
kubectl exec -it pod/redis-0 -n ains -- \
...
kubectl get configmap ains-config -n ains -o yaml

# 检查应用日志是否显示 AutoRouter 初始化
kubectl logs deployment/ains -n ains | grep -i "router\|read\|replica"
```

### Pod CrashLoopBackOff

```bash
kubectl describe pod ains-xxx -n ains | grep -A10 Events
kubectl logs pod/ains-xxx -n ains --previous
```
