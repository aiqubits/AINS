...
//! ## Token 存储策略
//!
//! JWT 和 refresh token 由后端通过 httpOnly cookie 下发（`ains_jwt`、
//! `ains_refresh`），浏览器自动管理，前端 JS 无法读取。
//!
//! 前端仅维护一个可读的 `ains_exp` cookie（存储 JWT 过期时间 Unix 秒），
//! 用于 UI 层的过期检测和自动刷新决策。
//!
...
impl AuthState {
    ...
    /// → 读取 `ains_exp` cookie 获取过期时间 → 检查是否过期
    ...
    pub async fn login(
        ...
        // 使用 refresh_expires_in 作为 Max-Age，确保 ains_exp 在 refresh token
