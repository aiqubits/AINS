---
version: "2.0"
name: AINS Admin UI Design System
description: >
...
AINS Admin 采用 **Indigo-Glass 毛玻璃设计语言**，通过三层质感体系营造轻盈、现代、专业的 BFF 管理后台体验：
...
- Logo 区域：高 80px（`h-20`），`px-6`，底部 `border-b border-slate-100`
  ...
  - 标题：`{typography.heading-lg}`，`bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent`（渐变色文字），文字 "AINS Admin"
  - 副标题：`{typography.fine-print}`，`{colors.text-muted}`，`tracking-wider uppercase`，文字 "Fullend System"
...
- 副标题：`{typography.caption}` (11px)，`{colors.text-muted}`，文字 "实时拦截捕获的 AINS 后端路由流量"
